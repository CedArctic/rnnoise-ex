...
for i=1:length(folders) %folders of noise types
    for j = 1:length(folders2) %folders of dB
        k = k+1;
        names = "RNNoise2\" + folders{i} + "\ " + folders2{j} +"\clean_wav"; %folder of enhanced signals
...
saveas(FigHandle,fullfile("resultsRNNoise",num2str(iFig)+".png"));
end
% FigList = findobj(allchild(0), 'flat', 'Type', 'figure');
% %FigList = flip(FigList);
% for iFig = 1:1:length(FigList)
% FigHandle = FigList(iFig);
% FigHandle.WindowState = 'maximized';
% saveas(FigHandle,fullfile("resultsRNNoiseModified_500.000",num2str(length(FigList)-iFig)+".png"));
% end